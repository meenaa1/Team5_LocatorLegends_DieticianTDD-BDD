// Generated from: tests\Features\DashboardPage.feature
import { test } from "playwright-bdd";

test.describe('Dashboard page', () => {

  test('Verify Navigation bar links presence', async ({ Given, When, Then }) => { 
    await Given('User is on the login page to login'); 
    await When('User clicks login button after entering with valid credentials'); 
    await Then('Navigation bar should display exactly four links "My Patients", "New Patient", "Login", and "Logout"'); 
  });

  test('Verify "My Patients" navigation link is clickable', async ({ Given, When, Then }) => { 
    await Given('User is on the Dashboard page'); 
    await When('User clicks the My Patients link in the navigation bar'); 
    await Then('User should be redirected to the "My Patients" page'); 
  });

  test('Verify "New Patient" navigation link is clickable', async ({ Given, When, Then }) => { 
    await Given('User is on the Dashboard page'); 
    await When('User clicks the New Patient link in the navigation bar'); 
    await Then('User should be redirected to the "New Patient" page'); 
  });

  test('Verify "Logout" navigation link is clickable', async ({ Given, When, Then }) => { 
    await Given('User is on the Dashboard page'); 
    await When('User clicks the Logout link in the navigation bar'); 
    await Then('User should be logged out of the application'); 
  });

  test('Verify Home icon is clickable', async ({ Given, When, Then }) => { 
    await Given('User is on the Dashboard page'); 
    await When('User clicks the Home icon on the navigation bar'); 
    await Then('User should be navigated to the dashboard page'); 
  });
...
test.use({
  ...
  $uri: [({}, use) => use('tests\\Features\\DashboardPage.feature'), { scope: 'test', box: true }],
