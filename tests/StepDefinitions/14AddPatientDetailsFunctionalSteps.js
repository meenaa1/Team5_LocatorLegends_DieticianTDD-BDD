...
Then('Values should be present inside Allergy dropdown', async ({}) => {
    await addPatientPage.dropdownOptions(addPatientPage.allergies).first().isVisible();
...
Then('Values should be present inside Food Preference dropdown', async ({}) => {
  await addPatientPage.dropdownOptions(addPatientPage.foodPreferences).first().isVisible();
...
Then('Values should be present inside Cuisine Category dropdown', async ({}) => {
  await addPatientPage.dropdownOptions(addPatientPage.cuisineCategory).first().isVisible(); 
...
When('User enters valid values in all field', async ({}) => {
  ...
  await addPatientPage.validPatientDetails(patientData);
